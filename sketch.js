...
var gamestate = 1;
var NeedyPeople,NeedyPeopleImage;
...
function preload(){
  ...
  NeedyPeopleImage = loadImage("Images/download (7).png");
...
function setup() {
  ...
  NeedyPeople = createSprite(displayWidth/2, displayHeight-250)
  NeedyPeople.addImage(NeedyPeopleImage);
  NeedyPeople.visible = false;
...
function draw() {
  ...
  if(gamestate == 2){
    textSize(30);
    fill("black");
    text("Here, Have Some Food, press enter to continue",displayWidth/2-160,displayHeight/2+150);
  }
  fill("green");
  textSize(30);
  text("Score : "+score,displayWidth/2-50,50);

  if(gamestate == 2 && keyIsDown(UP_ARROW)){
    console.log("hello");
    gamestate =1;
  }
...
function people(){
  ...
  FoodGroup.destroyEach();
  obstacleGroup.destroyEach();
  if(player.isTouching(NeedyPeople)) {
    player.velocityX = 0;
  }
}
